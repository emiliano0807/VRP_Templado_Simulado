...
def simulated_annealing(ruta, T_inicial):
    T = T_inicial
    T_MIN = 0.1
    V_enfriamiento = 100
    mejor_ruta = ruta[:]
...
def resolver_tsp():
    try:
        T = float(request.args.get('temperatura', 20))
    except ValueError:
        return jsonify({'error': 'Temperatura inválida'}), 400
    ...
    mejor_ruta, mejor_distancia = simulated_annealing(ciudades, T)
